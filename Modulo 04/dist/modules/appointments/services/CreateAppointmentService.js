...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    const appointmentDate = (0, _dateFns.startOfHour)(date); // se a data de agendamento for antes do que agora ele nao deixa realizar o agendamento
    ...
    const findAppointmentInSameDate = await this.appointmentsRepository.findByDate(appointmentDate, provider_id); // se o user id for o mesmo que o provider nao consegue fazer um agendamento com ele mesmo
    // caso a data ja esteja com agendamento ele da erro

    if (findAppointmentInSameDate) {
      throw new _AppError.default('This appointment is already booked', 401);
    }
